#https://github.com/zhoudaquan/rethinking_bottleneck_design/blob/master/mobilenext/codebase/models/mobilenetv2.py
...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio,kernel_size = 3):
        ...
        self.identity_div = 3
        self.sel_idx = inp // self.identity_div
    ...
    def forward(self, x):
        if self.identity:
            # shape = x.shape
            out = self.conv(x)
            # id_tensor = x[:,:self.sel_idx,:,:]
            # out[:,:self.sel_idx,:,:] = out[:,:self.sel_idx,:,:] + id_tensor
            return out + x#self.conv(x)
        else:
            return self.conv(x)
...
if __name__ == "__main__":
    ...
    ref_model = mobilenet_v2(num_classes=10, weights=None)  # This is the correct API for torchvision >= 0.13
    print("-------------------------Torchvision Efficient B0 Model----------------------")
