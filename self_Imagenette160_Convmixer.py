...
#from efficientnet_b0_2m import *
...
#model_1= ProgressiveModel(Conv_CBAM_Layer, num_layers=num_layers, dim=hidden_dim, patch_size=patch_size, kernel_size=kernel_size, n_classes=num_classes).to(device)   # First branch
#model_2 = ProgressiveModel(Conv_CBAM_Layer, num_layers=num_layers, dim=hidden_dim, patch_size=patch_size, kernel_size=kernel_size, n_classes=num_classes).to(device)   # First branch

model_1 = ConvMixerModel(dim=hidden_dim, depth=num_layers, patch_size=patch_size, kernel_size=kernel_size, n_classes=num_classes).to(device)
#model_2 = ConvMixerModel(dim=hidden_dim, depth=num_layers, patch_size=patch_size, kernel_size=kernel_size, n_classes=num_classes).to(device)

count_params_and_shapes(model_1, input_size=(1, 3, 160, 160))

load_model_wts(model_1, dataset, patch_size, hidden_dim, num_layers)
#load_model_wts(model_2, dataset, patch_size, hidden_dim, num_layers)
